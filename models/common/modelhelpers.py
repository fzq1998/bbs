...
class PostModelHelper(object):
    ...
    def post_list(cls,page,sort_type,board_id):
        ...
        if sort_type == cls.PostSortType.CREATE_TIME:
            ...
            posts = db.session.query(PostModel).outerjoin(PostStarModel).group_by(PostModel.id).order_by(db.func.count(PostStarModel.id).desc(), PostModel.create_time.desc())
